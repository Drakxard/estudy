...
export function ExerciseFeedbackButton({ id, exercise, response }: Props) {
  ...
  // 1) Exponer función global para parar el TTS
  useEffect(() => {
    window.stopTTS = () => {
      ...
      // Resetear estado
      setSpeaking(false);
      // Si usases <audio>, lo pausarías y reseteas:
      if (audioRef.current) {
...
// ——————— Aquí debajo ———————
// 2) Listener interno para Ctrl+P (parar) y Ctrl+O (leer)
useEffect(() => {
  const onKey = (e: KeyboardEvent) => {
    ...
    if (e.ctrlKey && e.key.toLowerCase() === 'o') {
      ...
      const cleaned = cleanText(raw)
        ...
        .replace(/\$[^$]+\$/g, '')
        .replace(/\*/g, '');
      renderSpeechFromMixedText(cleaned)
        .then(s => speakText(s))
        .catch(() => speakText(cleaned));
    }
  };

  document.addEventListener('keydown', onKey);
  return () => document.removeEventListener('keydown', onKey);
}, [summary, reasoning, showReasoning, cleanText]);
  const runFeedback = () => {
    mutation.mutate(
      { exercise, response },
      {
 ...
 <div className="relative mt-4 w-full p-4 bg-gray-800 rounded space-y-4">
     {/* audio oculto por si en el futuro generas blobs */}
     <audio id="tts-player" ref={audioRef} hidden />
  ...
  onClick={async () => {
    // Juntamos el texto bruto
    let raw = summary + (showReasoning && reasoning ? '\n' + reasoning : '');

    // 1) Limpiar etiquetas HTML
    let cleaned = cleanText(raw)
      // 2) Eliminar bloques de fórmulas $$…$$
      .replace(/\$\$[\s\S]*?\$\$/g, '')
      // 3) Eliminar fórmulas inline $…$
      .replace(/\$[^$]+\$/g, '')
      // 4) Eliminar asteriscos de énfasis Markdown
      .replace(/\*/g, '');

    try {
      // Renderizamos la voz a partir del texto limpio
      const speech = await renderSpeechFromMixedText(cleaned);
      console.log('[TTS] Invocando speakText con:', speech);
      speakText(speech);
    } catch (err) {
      console.error('[TTS] Error al renderizar voz:', err);
      // En fallback, usamos cleaned (ya sin HTML, fórmulas ni asteriscos)
      speakText(cleaned);
    }
  }}
  disabled={speaking}
